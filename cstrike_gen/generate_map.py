...
def generate(map_file_path, map_seed = None):

    if map_seed is None:
        map_seed = random.randint(0, 100000)

    print(f"Generating map from [{map_file_path}] using [{map_seed}]...")
    ...
    def filter_allowed_tiles(nx, ny, edge_id, opposite_edge_id, allowed_tiles):
        if nx < 0 or nx >= map_width or ny < 0 or ny >= map_length:
            ...
            #return [tile_id for tile_id in allowed_tiles if tiles[tile_id]["faces"][opposite_edge_id] == 0]
...
if __name__ == "__main__":
    ...
    if not os.path.exists(args.file_path):
        ...
        generate(args.file_path, args.seed)